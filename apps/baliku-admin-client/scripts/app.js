...
// Admin doesn't have role selection buttons - always uses 'admin' role
const loginRoleButtons = []; // Empty array - no role buttons in admin UI
...
let currentLoginRole = 'admin'; // Admin always uses 'admin' role
...
const normalize = (s) => String(s).toLowerCase().replace(/_/g, ' ').trim();
  ...
  try{
    const raw = localStorage.getItem('balikuAuthAdmin');
    if(!raw) return null;
  ...
  if(authUser){
    localStorage.setItem('balikuAuthAdmin', JSON.stringify(authUser));
  }
}
  function clearAuthStorage(){
  localStorage.removeItem('balikuAuthAdmin');
}
  ...
  showLoginScreen();
  // Admin always uses 'admin' role - load admin username options
  currentLoginRole = 'admin';
  loadLoginOptions('admin');
}
  async function initApp(){
  if(appInitialized) return;
  await init();
  // Admin sees all menus - no need to apply role-based visibility
  // applyRoleBasedMenuVisibility(); // Not needed for admin
  appInitialized = true;
}
  // Admin doesn't need setLoginRole - always uses 'admin' role
  // Function kept for compatibility but does nothing
  function setLoginRole(role){
    // Admin always uses 'admin' role - no role selection needed
    currentLoginRole = 'admin';
    if(loginNameInput){
  ...
  try{
    const res = await fetch(API_BASE + '/api/login', {
      ...
      body: JSON.stringify({ type: 'admin', name, password })
    });
    let data;
    if(!res.ok){
...
// Admin sees all menus - this function is kept for compatibility but does nothing
function applyRoleBasedMenuVisibility(){
  // Admin has full access - all menus are always visible
...
function applyRoleBasedFieldVisibility(){
  ...
  // Admin doesn't have role selection buttons - no event listeners needed
// loginRoleButtons is empty array, so this loop won't execute
